...
### 貪婪式策略挑選
1. 從策略池中，找最高目標函數的單一策略作為主體
2. 迭代加入互補策略（低相關 + 提升目標函數）
3. 直到無法改善 → 產出最終投組 P**
...
| Sortino | 只懲罰下方風險 |
| SQN | 系統品質 |
| Recovery Period | 回撤恢復時間 |
| Correlation | 策略間低相關 |
| Profit Factor | 毛利/毛損 |
| Win Rate × Payoff | 勝率×賺賠比 |

非單一指標 — 不同目標函數產出不同最優投組，最終人工判斷。

### 策略生命週期管理

```
LIVE → 監控 → 下架觸發？ → 備選庫 → 復活條件？ → LIVE
```

**下架觸發（雙條件）**：
1. 統計：近 3 月獲利 < 歷史 9/12/18/24 月均值的 60%
2. 體感：日損益落入 ±1.5-2.5σ 外，連續 3-5 天

下架 ≠ 淘汰。進備選庫持續監控，恢復正常可復活。
